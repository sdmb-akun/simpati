    ...
    tbody.innerHTML = siswaList.map(function(s) {
        ...
        return `
            <tr>
                ...
                <td>${s.kelas}</td>
                <td>${s.namaWali}</td>
                <td><a href="https://wa.me/${s.wa}" target="_blank" class="wa-link">${s.wa}</a></td>
                <td>${statusBadge}</td>
                <td class="action-cell">
                    <button class="btn-icon btn-sm" onclick="editSiswa('${s.nis}')" title="Edit">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn-icon btn-sm btn-danger" onclick="confirmDeleteSiswa('${s.nis}', '${s.nama}')" title="Hapus">
    ...
    if (search) {
        filtered = filtered.filter(function(s) {
            return s.nis.toLowerCase().includes(search) ||
                   ...
                   s.namaWali.toLowerCase().includes(search);
        });
    }
...
async function handleSaveSiswa(e) {
    ...
    let result;
    if (nisOld) {
        result = await apiCall("updateStudent", {
            nis: nisOld,
            nama: nama,
            kelas: kelas,
        ...
        result = await apiCall("saveStudent", {
            nis: nis,
            nama: nama,
            kelas: kelas,
            tahunAjaran: tahunAjaran,
    ...
    tbody.innerHTML = data.map(function(t) {
        return `
            <tr>
                ...
                <td>${formatCurrency(t.nominal)}</td>
                <td>${t.aktif === "TRUE" ? '<span class="badge badge-success">Aktif</span>' : '<span class="badge badge-danger">Nonaktif</span>'}</td>
                <td class="action-cell">
                    <button class="btn-icon btn-sm" onclick="editTagihanMaster('${t.id}')" title="Edit">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn-icon btn-sm btn-danger" onclick="confirmDeleteTagihanMaster('${t.id}', '${t.nama}')" title="Hapus">
                        <i class="fas fa-trash"></i>
                    </button>
                </td>
            </tr>
    ...
    tbody.innerHTML = data.map(function(t) {
        return `
            <tr>
                ...
                <td>${formatCurrency(t.terbayar)}</td>
                <td>${formatCurrency(t.sisa)}</td>
                <td>${getTagihanStatusBadge(t.status)}</td>
            </tr>
        `;
    }).join("");
}
    ...
    tbody.innerHTML = data.map(function(t) {
        return `
            <tr>
                ...
                <td>${formatCurrency(t.total)}</td>
                <td>${getTransaksiStatusBadge(t.status)}</td>
                <td class="action-cell">
...
async function handleSaveTransaksi(e) {
    ...
    checkboxes.forEach(function(cb) {
        tagihanList.push({
            tagihanSiswaId: cb.value,
            nominal: parseInt(cb.dataset.max)
        });
    });
...
async function confirmVoidTransaksi(id) {
    ...
    showLoading(true);
    const result = await apiCall("voidTransaction", {
        transaksiId: id,
    ...
    tbody.innerHTML = data.map(function(t) {
        return `
            <tr>
                ...
                <td class="action-cell">
                    <button class="btn btn-sm btn-success" onclick="verifikasiPembayaran('${t.id}')">
                        <i class="fas fa-check"></i> Verifikasi
                    </button>
                    <button class="btn btn-sm btn-danger" onclick="confirmVoidTransaksi('${t.id}')">
...
async function loadWhatsApp() {
    ...
    const [templateResult, logResult] = await Promise.all([
        apiCall("getWATemplate", {}, "GET"),
        apiCall("getWALog", {}, "GET")
    ]);
    ...
    tbody.innerHTML = data.map(function(t) {
        return `
            <tr>
                ...
                <td>${t.aktif === "TRUE" ? '<span class="badge badge-success">Aktif</span>' : '<span class="badge badge-danger">Nonaktif</span>'}</td>
                <td class="action-cell">
                    <button class="btn-icon btn-sm" onclick="editWATemplate('${t.id}')" title="Edit">
                        <i class="fas fa-edit"></i>
                    </button>
                </td>
            </tr>
    ...
    tbody.innerHTML = data.slice(0, 50).map(function(l) {
        return `
            <tr>
                ...
                <td>${l.noWa}</td>
                <td>${l.jenis}</td>
                <td>${l.status === "SUKSES" ? '<span class="badge badge-success">Sukses</span>' : '<span class="badge badge-danger">Gagal</span>'}</td>
            </tr>
        `;
    }).join("");
}
...
async function handleSaveWATemplate(e) {
    ...
    showLoading(true);
    const result = await apiCall("saveWATemplate", {
        id: id || undefined,
        nama: nama,
        isi: isi
    });
...
async function handleKirimReminderMassal() {
    ...
    // Panggil API sendWhatsapp untuk setiap siswa yang punya tagihan
    const siswaAktif = STATE.cache.siswa.filter(function(s) { return s.status === "AKTIF"; });
    
    for (let i = 0; i < siswaAktif.length; i++) {
        const s = siswaAktif[i];
        // Cek tagihan
        const result = await apiCall("getTagihanByNIS", { nis: s.nis }, "GET");
        
        if (result.success && result.data.length > 0) {
            const tunggakan = result.data.filter(function(t) { 
                return t.status !== "LUNAS" && t.jatuhTempo && new Date(t.jatuhTempo) < new Date();
            });
            
            // Kirim WhatsApp
            await apiCall("sendWhatsapp", {
                nis: s.nis,
                templateId: STATE.cache.waTemplate[0]?.id || ""
            });
        }
    }
            ...
            tbody.innerHTML = result.data.pemasukan.map(function(p) {
                return `
                    <tr>
                        ...
                        <td>${p.nis}</td>
                        <td>${p.namaSiswa || "-"}</td>
                        <td><span class="badge">${p.metode}</span></td>
            ...
            tbody.innerHTML = result.data.tunggakan.map(function(t) {
                return `
                    <tr>
                        ...
                        <td>${t.namaSiswa}</td>
                        <td>${t.kelas}</td>
                        <td>${t.namaWali} <br><a href="https://wa.me/${t.wa}" target="_blank" class="wa-link">${t.wa}</a></td>
                        <td>${t.namaTagihan}</td>
                        <td class="text-danger">${formatCurrency(t.sisa)}</td>
                        <td>${t.jatuhTempo}</td>
                    </tr>
                `;
            }).join("");
...
// Fungsi-fungsi yang dipanggil dari onclick HTML
